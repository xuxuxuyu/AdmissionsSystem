   ...
   */
   export default {
    ...
    methods: {
      ...
      // 处理模糊查询
      _handleFuzzyQueryValue() {
        let value = this.fuzzyQueryValue;
        // 转换模糊查询条件
        this.tablePagingParam.Searchs.PropertyParams = [];
        if (value) {
          let fuzzy = this.selectConfig.table.fuzzy || [];
          fuzzy &&
          fuzzy.forEach(item => {
            this.tablePagingParam.Searchs.PropertyParams.push({
                // 字段
                Field: item.Field,
                // 操作符
                Operation: this.globalDict.operation[item.Operation] || 0,
                // 逻辑
                Logic: this.globalDict.logic[item.Logic] || 1,
                // 值
                Value: value || ""
              });
          });
        }
        return this.tablePagingParam.Searchs.PropertyParams;
      },
      // 处理过滤条件
      _handleConditions() {
        this.tablePagingParam.Conditions.PropertyParams = [];
        // 遍历默认条件
        let arr = [];
        let where = this.selectConfig.table.where;
        if (where && Array.isArray(where)) {
          where.forEach((item, index) => {
            // @特殊处理 禁用标记
            if (
              item.Field === "StudentBasicInfo.BYBZ" ||
              item.Field === "JYBJ" ||
              item.Field === "JSBH"
              ) {
              arr.push({
                ...
                Field: item.Field,
                // 值
                Value: this.showColumnDisable,
                // 操作符
                Operation: this.globalDict.operation["Equal"],
                // 逻辑
                Logic: this.globalDict.logic["And"]
              });
          } else {
              // @特殊处理, 过滤异常值
              if (!this.searchForm[item.Field]) {
                return;
              }
              arr.push({
                // 字段
                Field: item.Field,
                // 值
                Value: this.searchForm[item.Field],
                // 操作符
                Operation: this.globalDict.operation[item.Operation] || 3, // 3 === Equal
                // 逻辑
                Logic: this.globalDict.logic[item.Logic] || 0 // 0 === And
              });
            }
          });
        }
        ...
        if (this.$refs.searchElement) {
          this.generateSearchConditions(conditions, this.$refs.searchElement);
        }
        this.tablePagingParam.Conditions.PropertyParams = [...arr, ...conditions];
        return this.tablePagingParam.Conditions.PropertyParams;
      },
      ...
      columnSearch(e, column) {
        ...
        if (e.keyCode == 13) {
          ...
          this.paramRequestList(this.searchData)
        }
      },
      handleDto(e, column){
        this.searchData[column.property] = e.target.value
      },
      async paramRequestList(params) {
        await this.$nextTick(); // conditions 的组装有时需要等待组件绘制完成。
        params.pageNum = this.currentPage
        params.pageCnt = this.pageSize
        this.$post(this.requestSuffix.select, {
          loadingEl: this.$refs.tableElement,
          data:{
            dto:params
